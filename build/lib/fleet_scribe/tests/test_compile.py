...
class TestDetectStablePatterns:
    ...
    def test_oscillating_data_not_trend(self):
        # Clearly non-monotonic data must not be classified as a trend.
        # _detect_trend requires >= 0.8 direction consistency; this sequence
        # alternates so consistency is well below that.
        history = [{"metric": v} for v in [5.0, 1.0, 5.0, 1.0, 5.0, 1.0]]
        patterns = detect_stable_patterns(history)
        trend_patterns = [p for p in patterns if p.is_trend() and p.key == "metric"]
        assert trend_patterns == []
    ...
    def test_cycle_detected_on_sine_wave(self):
        ...
        patterns = detect_stable_patterns(history)
        cycle_patterns = [p for p in patterns if p.is_cycle() and p.key == "wave"]
        assert len(cycle_patterns) == 1
        cyc = cycle_patterns[0]
        assert cyc.metadata["period"] == 4
        assert cyc.confidence >= 0.5
